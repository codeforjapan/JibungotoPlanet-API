...
const estimateHousing = async (
  ...
  if (housingAnswer.housingAmountByRegionFirstKey) {
    const housingAmountByRegion = housingAnswer.housingAmountByRegionFirstKey + '_';
    const params = {
      TableName: parameterTableName,
  ...
  if (housingAnswer.housingSizeKey) {
    const housingSize = await getData('housing-size', housingAnswer.housingSizeKey);
  ...
  if (housingAnswer.electricityIntensityKey) {
    const electricityParam = await getData('electricity-intensity', housingAnswer.electricityIntensityKey);
  ...
  if (
    ...
    if (
      ...
      const chargingData = await getData('car-charging', mobilityAnswer.carChargingKey);
    ...
    estimationAmount.electricity.value =
        (housingAnswer.electricityMonthlyConsumption * electricitySeason.Item?.value) /
        residentCount -
        mobilityElectricityAmount;
  ...
  if (housingAnswer.useGas) {
    ...
    if (
      ...
      const gasSeason = await getData('gas-season-factor', housingAnswer.gasSeasonFactorKey);
      const gasFactor = await getData('energy-heat-intensity', housingAnswer.energyHeatIntensityKey);

      gasParam =
          (housingAnswer.gasMonthlyConsumption * (gasSeason.Item?.value || 1) * (gasFactor.Item?.value || 1)) /
          residentCount;
    }
  ...
  if (housingAnswer.useKerosene) {
    if (
      ...
      estimationAmount.kerosene.value =
          ((keroseneData?.Item?.value || 1) * (housingAnswer.keroseneMonthlyConsumption * housingAnswer.keroseneMonthCount)) /
          residentCount;
    }
