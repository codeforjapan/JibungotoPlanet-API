...
const estimateMobility = async (
  ...
  const pushOrUpdateEstimate = (
      item: any,
      type: any,
      estimation: { domain?: any; item?: any; type?: any; value: any; subdomain?: any; unit?: any }
  ) => {
  ...
  if (housingAnswer?.electricityIntensityKey) {
    const electricityData = await getData('electricity-intensity-factor', housingAnswer.electricityIntensityKey);
    if (electricityData?.Item) {
      electricityIntensityFactor = electricityData.Item.value;
    }

    const carChargingData = await getData('car-charging', mobilityAnswer?.carChargingKey || 'unknown');
  ...
  if (mobilityAnswer.hasPrivateCar) {
    if (mobilityAnswer?.carIntensityFactorFirstKey) {
      ...
      // PHV, EVの補正
      if (mobilityAnswer.carIntensityFactorFirstKey === 'phv' || mobilityAnswer.carIntensityFactorFirstKey === 'ev') {
        ...
        if (data?.Item) {
          ghgIntensityRatio =
              ghgIntensityRatio * (1 - electricityIntensityFactor) + data.Item.value * electricityIntensityFactor;
        }
      }

      // 人数補正値
      data = await getData('car-passengers', (mobilityAnswer.carPassengersFirstKey || 'unknown') + '_private-car-factor');
      let passengerIntensityRatio = data?.Item?.value || 1;

      const purchaseIntensity = createIntensity(baselines, 'private-car-purchase');

      // 自家用車の場合は、自動車種類に応じて運転時GHG原単位を取得
      const purchaseData = await getData(
          'car-intensity-factor',
          (mobilityAnswer.carIntensityFactorFirstKey || 'unknown') + '_manufacturing-factor'
      );
      ...
      const privateCarMaintenance = createAmount(baselines, 'private-car-maintenance');
  ...
  if (mobilityAnswer.carPassengersFirstKey) {
    ...
    // 人数補正値
    const data = await getData('car-passengers', mobilityAnswer.carPassengersFirstKey + '_taxi-factor');
  ...
  if (mobilityAnswer.carPassengersFirstKey && mobilityAnswer.carIntensityFactorFirstKey) {
    ...
    const driving = await getData(
        'car-intensity-factor',
        (mobilityAnswer.carIntensityFactorFirstKey || 'unknown') + '_driving-factor'
    );
  ...
  if (mobilityAnswer.hasTravelingTime) {
    ...
    const motorbikeDrivingRatio = estimationAmount.motorbike.value / baselineMotorbikeAmount;
    const carSharingDrivingRatio = estimationAmount.carSharing.value / baselineCarSharingAmount;
    ...
    // お住まいの地域の規模はどのくらいですか？
    //
    const mileageByAreaFirstKey = mobilityAnswer.mileageByAreaFirstKey || 'unknown';
    const params = {
      TableName: parameterTableName,
      KeyConditionExpression: 'category = :category and begins_with(#key, :key)',
    ...
    estimationAmount.airplane.value = consumptionByArea[mileageByAreaFirstKey + '_airplane'];
    estimationAmount.train.value = consumptionByArea[mileageByAreaFirstKey + '_train'];
    estimationAmount.bus.value = consumptionByArea[mileageByAreaFirstKey + '_bus'];
    estimationAmount.motorbike.value = consumptionByArea[mileageByAreaFirstKey + '_motorbike-driving'];
    estimationAmount.taxi.value = consumptionByArea[mileageByAreaFirstKey + '_taxi'];
    estimationAmount.carSharing.value = consumptionByArea[mileageByAreaFirstKey + '_car-sharing-driving'];
    estimationAmount.ferry.value = consumptionByArea[mileageByAreaFirstKey + '_ferry'];
    estimationAmount.motorbikePurchase.value = consumptionByArea[mileageByAreaFirstKey + '_motorbike-purchase'];
    estimationAmount.carSharingRental.value = consumptionByArea[mileageByAreaFirstKey + '_car-sharing-rental'];
    estimationAmount.motorbikeMaintenance.value = consumptionByArea[mileageByAreaFirstKey + '_motorbike-maintenance'];
    ...
    additionalAmount.walking.value = consumptionByArea[mileageByAreaFirstKey + '_walking'];
    additionalAmount.bicycleDriving.value = consumptionByArea[mileageByAreaFirstKey + '_bicycle-driving'];
    additionalAmount.bicycleMaintenance.value = consumptionByArea[mileageByAreaFirstKey + '_bicycle-maintenance'];
