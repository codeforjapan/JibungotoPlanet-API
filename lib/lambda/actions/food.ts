...
const estimateFood = async (
  ...
  // foodのベースラインの取得
  const createAmount = (baselines: any, item: string) => toEstimation(findBaseline(baselines, 'food', item, 'amount'));
  const createIntensity = (baselines: any, item: string) => toEstimation(findBaseline(baselines, 'food', item, 'intensity'));
  const getCategoryBaseTotal = (baselines: any, item: string) => findBaseline(baselines, 'food', item, 'amount').value * findBaseline(baselines, 'food', item, 'intensity').value;
  const getCategoryCustomTotal = (baselines: any, item: string, value: number) => value * findBaseline(baselines, 'food', item, 'intensity').value;
  ...
  const foodIntakeFactor = await getData('food-intake-factor', foodAnswer?.foodIntakeFactorKey || 'unknown');
  ...
  if (foodAnswer?.foodDirectWasteFactorKey && foodAnswer?.foodLeftoverFactorKey) {
    const foodDirectWasteFactor = await getData('food-direct-waste-factor', foodAnswer.foodDirectWasteFactorKey);
    const foodLeftoverFactor = await getData('food-leftover-factor', foodAnswer.foodLeftoverFactorKey);

    const foodWasteRatioData = await dynamodb.send(new QueryCommand({
      TableName: parameterTableName,
    ...
    // @ts-ignore
    const leftoverRatio = foodWasteRatioData.Items.find((item: { key: string; }) => item.key === 'leftover-per-food-waste');
    // @ts-ignore
    const directWasteRatio = foodWasteRatioData?.Items.find((item: { key: string; }) => item.key === 'direct-waste-per-food-waste');
    // @ts-ignore
    const foodWasteRatio = foodWasteRatioData?.Items.find((item: { key: string; }) => item.key === 'food-waste-per-food');
    ...
    estimationAmount.rice.value *= foodIntakeFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
    ...
    estimationAmount.vegetables.value *= foodIntakeFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
    ...
    estimationAmount.oil.value *= foodIntakeFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
    ...
    if (foodAnswer?.dairyFoodFactorKey) {
      const dairyFoodFactor = await getData('dairy-food-factor', foodAnswer.dairyFoodFactorKey);

      estimationAmount.milk.value *= dairyFoodFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
      estimationAmount['other-dairy'].value *= dairyFoodFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
      estimationAmount.eggs.value *= dairyFoodFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
    ...
    if (foodAnswer?.dishBeefFactorKey) {
      dishBeefFactor = await getData('dish-beef-factor', foodAnswer.dishBeefFactorKey);

      estimationAmount.beef.value *= dishBeefFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
    ...
    if (foodAnswer?.dishPorkFactorKey) {
      dishPorkFactor = await getData('dish-pork-factor', foodAnswer.dishPorkFactorKey);

      estimationAmount.pork.value *= dishPorkFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
    ...
    if (foodAnswer?.dishChickenFactorKey) {
      ...
      estimationAmount.chicken.value *= dishChickenFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
    ...
    if (foodAnswer?.dishSeafoodFactorKey) {
      const dishSeafoodFactor = await getData('dish-seafood-factor', foodAnswer.dishSeafoodFactorKey);

      estimationAmount.fish.value *= dishSeafoodFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
    ...
    if (foodAnswer?.alcoholFactorKey) {
      const alcoholFactor = await getData('alcohol-factor', foodAnswer.alcoholFactorKey);

      estimationAmount.alcohol.value *= alcoholFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
      estimations.push(estimationAmount.alcohol);
    }

    // 菓子など補正
    if (foodAnswer?.softDrinkSnackFactorKey) {
      const softDrinkSnackFactor = await getData('soft-drink-snack-factor', foodAnswer.softDrinkSnackFactorKey);

      estimationAmount['sweets-snack'].value *= softDrinkSnackFactor.Item?.value * foodPurchaseAmountConsideringFoodLossRatio;
    ...
    readyMealIntensity.value =
        (readyMealIntensity.value *
            (beforeReadyMealKeyArray.reduce(
                    // @ts-ignore
                    (res, key) => res + getCategoryCustomTotal(baselines, key, estimationAmount[key].value),
                    0
                ) /
                currentTotalAmount)) /
        (beforeReadyMealKeyArray.reduce(
    ...
    if (foodAnswer?.eatOutFactorKey) {
      const eatOutFactor = await getData('eat-out-factor', foodAnswer.eatOutFactorKey);
      ...
      const eatOutIntensityResult =
          EatOutArray.reduce((res, key) => {
            if (key !== 'ready-meal') {
              // @ts-ignore
              return res + getCategoryCustomTotal(baselines, key, estimationAmount[key].value);
            } else {
              return res + estimationAmount[key].value * readyMealIntensity.value;
            }
          }, 0) /
          currentTotalAmount /
          (EatOutArray.reduce((res, key) => res + getCategoryBaseTotal(baselines,key), 0) / baseTotalAmount);
